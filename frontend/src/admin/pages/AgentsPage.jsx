...
import { Plus, Pencil, Trash2, Users, BadgeCheck } from 'lucide-react'
...
export default function AgentsPage() {
  ...
  const remove = async (id) => {
    ...
    await adminAgents.delete(id); load()
  }

  const cols = [
    { key: 'name', label: 'Agent', render: (r) => (
      <div className="flex items-center gap-2">
        <div className="w-8 h-8 rounded-full bg-[#0B1F3A] flex items-center justify-center text-white text-xs font-bold">{r.name?.[0]}</div>
        <div>
