...
export default function AppUpdatePage() {
  ...
  const handleFile = (f) => {
    ...
    if (!f.name.endsWith('.zip')) {
      setResult({ error: true, message: 'Only .zip files are accepted.' })
      return
    }
    setFile(f)
    setResult(null)
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto space-y-8">
      ...
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-gray-900">App Update Manager</h1>
        <p className="text-sm text-gray-500 mt-1">
          Upload a <code className="bg-gray-100 px-1 rounded">.zip</code> archive to deploy new files to the web application.
          PHP files and shell scripts are blocked for security.
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-5">
        ...
        >
          ...
          {file ? (
            <div className="space-y-1">
              ...
              <p className="text-xs text-[#730D26] mt-1">Click or drop to replace</p>
            </div>
          ) : (
            <div className="space-y-1">
              <p className="font-medium text-gray-700">Drop your <span className="text-[#730D26]">.zip</span> file here</p>
              <p className="text-sm text-gray-400">or click to browse — max 512 MB</p>
            </div>
          )}
        </div>
        ...
        {uploading && (
          <div className="space-y-1.5">
            ...
            <div className="w-full bg-gray-100 rounded-full h-2">
              <div
                className="bg-[#730D26] h-2 rounded-full transition-all duration-300"
            ...
            <div>
              ...
              {result.data && (
                <p className="text-xs mt-0.5 opacity-75">
                  {result.data.files_extracted} file(s) extracted
                  {result.data.skipped > 0 ? `, ${result.data.skipped} skipped (security)` : ''}
                </p>
              )}
            </div>
          </div>
        ...
        <div className="flex items-start gap-2 text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
          ...
          <span>
            For security, <strong>.php</strong>, <strong>.sh</strong>, <strong>.env</strong>, and executable files inside the ZIP are automatically blocked and will not be extracted.
          </span>
        </div>
        ...
        >
          {uploading ? 'Applying Update…' : 'Apply Update'}
        </button>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100">
          ...
          <button
            ...
            className="text-gray-400 hover:text-[#730D26] transition-colors"
            title="Refresh"
          >
        ...
        {loadingHistory ? (
          ...
          <div className="p-10 text-center text-gray-400 text-sm">No updates have been applied yet.</div>
        ) : (
          <table className="w-full text-sm">
            <thead>
              <tr className="text-left text-xs font-semibold text-gray-400 uppercase tracking-wider bg-gray-50">
                ...
                <th className="px-6 py-3 hidden lg:table-cell">Applied by</th>
              ...
              {history.map((row) => (
                <tr key={row.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-6 py-4">
                    ...
                    {row.note && (
                      <p className="text-xs text-amber-600 mt-0.5 ml-5">{row.note}</p>
                    )}
                  </td>
