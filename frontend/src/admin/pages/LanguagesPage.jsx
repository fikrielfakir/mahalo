...
function LanguageForm({ initial, onSave, onCancel, saving }) {
  ...
  return (
    <div className="bg-blue-50 border border-blue-100 rounded-2xl p-5 space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-xs font-semibold text-gray-500 mb-1">Code <span className="text-red-400">*</span></label>
          <input
            value={form.code}
            onChange={e => set('code', e.target.value.toLowerCase())}
            placeholder="e.g. fr"
            maxLength={10}
            className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:border-[#BA1932]"
          />
        </div>
        <div>
          <label className="block text-xs font-semibold text-gray-500 mb-1">Label <span className="text-red-400">*</span></label>
          <input
            value={form.label}
        ...
        <div>
          <label className="block text-xs font-semibold text-gray-500 mb-1">Native Label</label>
          <input
            value={form.native_label}
            onChange={e => set('native_label', e.target.value)}
        ...
        <div>
          <label className="block text-xs font-semibold text-gray-500 mb-1">MyMemory code</label>
          <input
            value={form.mymemory_code}
            onChange={e => set('mymemory_code', e.target.value.toLowerCase())}
            placeholder="e.g. fr (for auto-translate)"
            className="w-full text-sm border border-gray-200 rounded-xl px-3 py-2 focus:outline-none focus:border-[#BA1932]"
          />
        </div>
        <div>
      ...
      <div className="flex items-center gap-6">
        <label className="flex items-center gap-2 cursor-pointer select-none">
          ...
          <span className="text-sm text-gray-700">Right-to-left (RTL)</span>
        </label>
        <label className="flex items-center gap-2 cursor-pointer select-none">
      ...
      <div className="flex gap-2 pt-1">
        <button
          ...
          className="flex items-center gap-2 px-4 py-2 bg-[#BA1932] hover:bg-[#9a1525] text-white text-sm font-semibold rounded-xl transition-colors disabled:opacity-50"
        >
          <Check size={14} />
          {saving ? 'Saving…' : 'Save'}
        </button>
        <button
          onClick={onCancel}
          className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-200 text-gray-600 text-sm rounded-xl hover:bg-gray-50 transition-colors"
        >
          <X size={14} />
          Cancel
        </button>
...
export default function LanguagesPage() {
  ...
  const handleCreate = async (form) => {
    ...
    try {
      ...
      setAdding(false)
      setToast({ message: 'Language added', type: 'success' })
      load()
    } catch (e) {
      setToast({ message: e?.message || 'Failed to create', type: 'error' })
    } finally {
      setSaving(false)
    }
  }
  ...
  const handleDelete = async (id, label) => {
    ...
    try {
      ...
      setToast({ message: 'Language deleted', type: 'success' })
      load()
    } catch {
  ...
  const handleToggleActive = async (lang) => {
    try {
      ...
      setToast({ message: lang.is_active ? 'Language disabled' : 'Language enabled', type: 'success' })
      load()
    } catch {
    ...
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900">Languages</h1>
            <p className="text-sm text-gray-500">Manage supported languages for translations</p>
          </div>
        </div>
        {!adding && (
          <button
      ...
      {adding && (
        <div className="mb-4">
          <LanguageForm onSave={handleCreate} onCancel={() => setAdding(false)} saving={saving} />
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <div className="py-16 text-center text-gray-400 text-sm">No languages configured</div>
        ) : (
          <table className="w-full text-left">
            <thead className="bg-gray-50 border-b border-gray-100">
              <tr>
                ...
                <th className="px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">MyMemory</th>
                <th className="px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Options</th>
                <th className="px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider">Status</th>
                <th className="px-4 py-3 text-xs font-semibold text-gray-400 uppercase tracking-wider w-28">Actions</th>
              </tr>
            </thead>
            <tbody>
              {languages.map(lang => (
                editId === lang.id ? (
                  <tr key={lang.id}>
                    <td colSpan={7} className="px-4 py-3">
                      <LanguageForm
                        initial={{ ...lang, is_rtl: !!lang.is_rtl, is_active: !!lang.is_active }}
                        onSave={handleUpdate}
                  ...
                  <tr key={lang.id} className="border-b border-gray-100 hover:bg-gray-50 group">
                    ...
                    <td className="px-4 py-3">
                      <button
                        ...
                        title={lang.is_active ? 'Click to disable' : 'Click to enable'}
                      >
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          onClick={() => { setEditId(lang.id); setAdding(false) }}
                        ...
                        <button
                          onClick={() => handleDelete(lang.id, lang.label)}
      ...
      <p className="text-xs text-gray-400 px-1 mt-3">
        Active languages appear in the Translation Manager and content translation modals. Disabled languages are hidden from the UI.
      </p>
    </div>
  )
}
