...
const EMPTY = { name: '', description: '', content: '', location: '', images: '', investor_id: '', city_id: '', price_from: '', price_to: '', is_featured: false, status: 'selling' }
...
export default function ProjectsPage() {
  ...
  const openEdit   = (row) => { setEditing(row); setForm({ ...row, images: (row.images || []).join('\n') }); setModal(true) }
  const f = (k) => (e) => setForm((p) => ({ ...p, [k]: e.target?.value ?? e }))

  const submit = async (e) => {
    e.preventDefault(); setSaving(true)
    try {
      const payload = { ...form, images: form.images ? form.images.split('\n').map(s=>s.trim()).filter(Boolean) : [], price_from: form.price_from || null, price_to: form.price_to || null, city_id: form.city_id || null, investor_id: form.investor_id || null }
  ...
  const cols = [
    { key: 'name', label: 'Project', render: (r) => (
      <div className="flex items-center gap-2">
        <div className="w-8 h-8 rounded-xl bg-[#0B1F3A]/5 flex items-center justify-center"><FolderKanban size={14} className="text-[#0B1F3A]/40" /></div>
        <div><p className="font-medium text-sm text-gray-800">{r.name}</p><p className="text-xs text-gray-400">{r.city?.name || '—'}</p></div>
      </div>
    )},
    ...
    <div>
      ...
      <Modal open={modal} onClose={() => setModal(false)} title={editing ? 'Edit Project' : 'Add Project'} size="lg">
        <form onSubmit={submit} className="space-y-4">
          <FormField label="Project Name" required><Input value={form.name} onChange={f('name')} required placeholder="The View Anfa" /></FormField>
          <div className="grid grid-cols-2 gap-4">
            ...
            <FormField label="Price From (MAD)"><Input type="number" value={form.price_from} onChange={f('price_from')} placeholder="1500000" /></FormField>
            <FormField label="Price To (MAD)"><Input type="number" value={form.price_to} onChange={f('price_to')} placeholder="5000000" /></FormField>
          ...
          <FormField label="Location"><Input value={form.location} onChange={f('location')} placeholder="Anfa, Casablanca" /></FormField>
          <FormField label="Description"><Textarea value={form.description} onChange={f('description')} rows={2} placeholder="Short description..." /></FormField>
          <FormField label="Images (one URL per line)" hint="One image URL per line"><Textarea value={form.images} onChange={f('images')} rows={3} placeholder="https://images.unsplash.com/..." /></FormField>
