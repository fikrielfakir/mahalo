...
const ROWS = [
  ...
  { label: 'Type',       render: p => p.type === 'rent' ? 'For Rent' : 'For Sale' },
  { label: 'City',       render: p => p.city?.name || '—' },
  { label: 'Category',   render: p => p.category?.name || '—' },
  { label: 'Bedrooms',   render: p => p.number_bedroom || '—' },
  { label: 'Bathrooms',  render: p => p.number_bathroom || '—' },
  { label: 'Area',       render: p => p.square ? `${p.square} m²` : '—' },
  { label: 'Verified',   render: p => p.is_verified ? '✓ Yes' : 'No' },
  { label: 'Featured',   render: p => p.is_featured ? '✓ Yes' : 'No' },
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 shrink-0">
              <div className="flex items-center gap-2">
                ...
                <h2 className="font-bold text-navy text-lg">Property Comparison</h2>
              </div>
            ...
            <div className="overflow-auto flex-1">
              <table className="w-full">
                ...
                <thead>
                  <tr>
                    <th className="w-36 p-4 text-left text-xs font-semibold text-gray-400 uppercase tracking-wide sticky left-0 bg-white">Attribute</th>
                    {list.map(p => (
                    ...
                    {Array.from({ length: MAX - list.length }).map((_, i) => (
                      <th key={i} className="p-4 min-w-[180px]">
                        <div className="aspect-video rounded-2xl border-2 border-dashed border-gray-200 flex items-center justify-center mb-3">
                          <span className="text-gray-300 text-xs">Add property</span>
                        </div>
                      </th>
                    ))}
                  </tr>
